...
test("throws TypeError for primitive string value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", "string"), { instanceOf: TypeError });
});

test("throws TypeError for primitive number value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", 42), { instanceOf: TypeError });
});

test("throws TypeError for primitive boolean value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", true), { instanceOf: TypeError });
});

test("throws TypeError for null value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", null), { instanceOf: TypeError });
});

test("throws TypeError for undefined value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", undefined), { instanceOf: TypeError });
});

test("throws TypeError for symbol value", (t) => {
  const cache = new WeakCache();
  t.throws(() => cache.set("key", Symbol("test")), { instanceOf: TypeError });
});

test("allows function values", (t) => {
  const cache = new WeakCache();
  const function_ = () => {
    // Function values do not need to perform work for this cache test.
  };
